...
const REGISTRY = {
  PreToolUse: {
    Bash: [
      ...
      ['secrets', secrets],
      // `deploy` TRƯỚC `infra`: `vercel --prod` bị cả hai bắt, và ruleId được báo
      // chính là khoá escape người dùng sẽ gõ. `deploy.direct-tool` nói "hãy dùng
      // script", `infra.deny-binary` chỉ nói "binary bị chặn" — cái đầu hành động
      // được. Sau `secrets` vì secrets rẻ hơn và nghiêm trọng hơn.
      //
      // KHÔNG khai cho `apply_patch`: nhóm này soi lệnh sẽ chạy, mà ctx.command là
      // null với apply_patch (context.mjs) nên nó chỉ trả ALLOW ngay — thêm vào
      // chỉ tốn một lần gọi hàm trên đường nóng.
      ['deploy', deploy],
      ['infra', infra],
      ['git', gitWorkflow],
    ],
