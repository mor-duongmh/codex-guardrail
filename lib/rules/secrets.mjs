...
// Matches: PASSWORD, TOKEN, SECRET, APIKEY (bare or with underscores)
// Plus AWS_ prefix separately
...
export function evaluate(ctx, policy) {
  ...
  for (const segment of splitSegments(ctx.command)) {
    ...
    // Also strip command substitution wrappers like $(cmd), `cmd`, (cmd)
    ...
    if (firstWord === 'env') {
      // Check if env is wrapped in command substitution ($(env), `env`, (env))
      if (/^\s*[\$`(]/.test(trimmed) && /[\)}`]$/.test(trimmed.trim())) {
        return deny('secrets.env-dump',
          `"env" bị bọc trong command substitution sẽ xả toàn bộ biến môi trường, trong đó có thể có secret.`,
          'Chạy command trực tiếp thay vì wrapping, hoặc lọc biến cụ thể.');
      }
