...
import { effectiveArgv, stripGroupSuffix } from '../argv.mjs';
...
// Đường dẫn có thể nằm CHUNG token với cờ, và khi đó vòng quét token ở dưới
// không thấy: đo được 2026-09-04 rằng `dd if=.env of=/tmp/leak`,
// `grep --file=.env x`, `tar --file=.env -c` đều LỌT. Đây là CÙNG cái bẫy như
// dấu nhóm: pattern cây (`~/.aws/**`) vẫn chặn được vì `**` hút cả tiền tố
// `if=`, nên chỉ pattern tên file CHÍNH XÁC (`**/.env`) hở.
//
// Hai chỗ cố ý KHÔNG bóc, để không mua an toàn bằng chặn oan:
//   - `--exclude=` / `--ignore=`: đó là cờ BẢO VỆ (`tar --exclude=.env`). Chặn
//     một lệnh đang tránh secret là dạy dev tắt guardrail.
//   - gán biến môi trường (`ENV_FILE=.env npm start`): bản thân nó không đọc
//     file, và đó là workflow dotenv thật. Lỗ có chủ ý, ghi ở README.
// `if=`/`of=` liệt kê tay vì đó là cú pháp operand riêng của `dd`: không có
// dấu gạch nên không nhận ra được bằng hình dạng.
const PROTECTIVE_FLAG = /^--(exclude|ignore)=/;

function flagValuePath(token) {
  if (PROTECTIVE_FLAG.test(token)) return null;
  if (token.startsWith('-')) {
    const eq = token.indexOf('=');
    return eq > 0 ? token.slice(eq + 1) : null;
  }
  return /^(if|of)=/.test(token) ? token.slice(3) : null;
}
...
export function evaluate(ctx, policy) {
  ...
  for (const sub of parseCommand(ctx.command)) {
    for (const rawToken of sub.argv.slice(1)) {
      // Dấu `)`/`}`/`;` của nhóm lệnh dính vào token cuối vì tokenize không coi
      // chúng là ranh giới, và hệ quả KHÔNG ĐỀU: pattern kết bằng tên file chính
      // xác (`**/.env`, `**/id_rsa`) bị lách, còn pattern cây `**` (`~/.aws/**`)
      // vẫn chặn được vì `**` hút luôn dấu đó. Tức lỗ chỉ hở đúng những pattern
      // chính xác nhất — dễ tin là đã an toàn. Đo được: `(cat .env)` và
      // `(cd app && cat .env)` LỌT, `(cat ~/.aws/credentials)` thì không.
      //
      // Bóc theo TỪNG TOKEN chứ không đổi sang `effectiveArgv`: ở đây đang quét
      // MỌI token nên chỉ cần chuẩn hoá, còn đổi nguồn token sẽ BỎ ĐI dải wrapper
      // và mất một token khi lệnh chỉ còn đúng một tham số.
      // Dấu bóc cũng sửa luôn chặn oan chiều ngược lại: `(cat .env.example)`
      // trước đây bị chặn vì `.env.example)` khớp deny mà không khớp allow.
      const token = stripGroupSuffix(rawToken) || rawToken;
      if (isSensitive(token)) {
        return deny('secrets.read-path',
          `Lệnh chạm đường dẫn nhạy cảm "${token}".`, HINT);
      }
      const inFlag = flagValuePath(token);
      if (inFlag && isSensitive(inFlag)) {
        return deny('secrets.read-path',
          `Lệnh chạm đường dẫn nhạy cảm "${inFlag}" trong "${token}".`, HINT);
      }
    }
  }
