...
// giới: `(cd /r && git push --force)` cho token `--force)`, còn
// `(cd /r && git push)` cho `push)`. Cả cờ và subcommand đều lệch, nên so khớp
// chính xác trượt. Trong shell thật dấu đó KHÔNG thuộc tham số nên bóc là đúng
// nghĩa; nếu nó thật sự nằm trong message (`-m "abc (1)"`) thì chỉ lệch phần
// hiển thị, không đổi verdict vì pattern message kết bằng `.+`.
...
function parseGitArgs(args) {
  ...
  return { verb, flags, message, repoDirs };
}
...
export function evaluate(ctx, policy, deps = {}) {
  ...
  for (const sub of parseCommand(ctx.command)) {
    ...
    const { verb, flags, message, repoDirs } = parseGitArgs(args);
    ...
    if (verb === 'push'
      && hasFlag(flags, 'fd', ['--force', '--force-with-lease', '--delete'])) {
    ...
    if (verb === 'commit' && message !== null && (cfg.commitMessagePattern ?? '') !== '') {
      if (!new RegExp(cfg.commitMessagePattern).test(message)) {
        return deny('git.commit-message',
          `Message "${message}" không khớp quy ước commit của dự án.`,
