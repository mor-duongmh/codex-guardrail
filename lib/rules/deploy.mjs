...
// Cờ của curl nghĩa là ĐẨY dữ liệu ra. Tách "đẩy lên" khỏi "lấy về" là tín hiệu
// giảm số prompt nhiều nhất: tải-về là việc thường xuyên, đẩy-lên thì hiếm. Không
// tách thì rule này hỏi ở mọi `curl`, và prompt fatigue huấn luyện dev bấm qua cả
// confirm deploy — phá chính cơ chế nó phục vụ.
...
// scp/rsync: đích là tham số CUỐI. `host:path` ở cuối = đẩy lên, `host:path` ở
// đầu = lấy về. Đây là thứ duy nhất phân biệt hai chiều, nên nó không được lấy
// host đầu tiên tìm thấy (khác `scpHosts` của infra, thứ cố ý lấy MỌI host vì
// deny-list phải chặn cả chiều đọc).
...
// URL không đọc được -> null, tức KHÔNG hỏi. `curl -X POST "$ENDPOINT"` không nở
// biến, nên hỏi về nó là một prompt không hành động được — và nó chỉ dạy dev bấm
// qua. Đây là chỗ cố ý lệch về phía CHO QUA, ngược với mọi chỗ khác trong nhóm:
// một prompt vô nghĩa làm hỏng cơ chế nhiều hơn một lệnh lọt.
...
export function evaluate(ctx, policy, currentBranch = defaultCurrentBranch) {
  ...
  // Biên dịch NGOÀI vòng lặp segment. `denyDirect` mặc định rỗng nên đường nóng
  // không trả phí gì.
  ...
  for (const sub of parseCommand(ctx.command)) {
    ...
    // `grep -rn mydeploy docs/`) không bao giờ khớp.
    //
    // Chạy TRƯỚC matchEntry: nếu một công cụ vừa khớp denyDirect vừa là entrypoint
    // thì denyDirect thắng — dự án đã tuyên bố nó không được gọi trực tiếp.
    ...
    // Nhóm B: hỏi, không chặn. Chỉ chạy khi dự án ĐÃ khai declaredHosts — chưa
    // khai thì im lặng hoàn toàn, vì hỏi trên một dự án chưa cấu hình là cách
    // nhanh nhất để dev học phản xạ bấm OK.
    if (declaredRes.length > 0) {
      const bin = basename(argv[0]);
      let host = null;
      if (bin === 'scp' || bin === 'rsync') host = remoteDestHost(argv);
      // `ssh host "..."` là CHẠY LỆNH từ xa, không phải đọc — nên luôn tính là đẩy
