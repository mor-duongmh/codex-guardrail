import { appendFileSync, mkdirSync, readFileSync, existsSync, chmodSync } from 'node:fs';
import { homedir } from 'node:os';
import { dirname, join } from 'node:path';
import { redact } from './redact.mjs';

export function auditPath() {
  return process.env.GUARDRAIL_AUDIT_PATH
    ?? join(homedir(), '.codex', 'guardrail-audit.jsonl');
}

export function record(entry) {
  try {
    const payload = { ts: new Date().toISOString(), ...entry };
    if (payload.command) payload.command = redact(payload.command);
    const p = auditPath();
    const dir = dirname(p);
    mkdirSync(dir, { recursive: true, mode: 0o700 });
    // Ensure directory has strict permissions (handles case where it already existed)
    try { chmodSync(dir, 0o700); } catch { /* chmod fail-open */ }
    appendFileSync(p, JSON.stringify(payload) + '\n', { mode: 0o600 });
    // Ensure file has strict permissions (handles case where it already existed with loose perms)
    try { chmodSync(p, 0o600); } catch { /* chmod fail-open */ }
  } catch {
    // Ghi log thất bại không được chặn công việc của dev.
  }
}

export function readEntries() {
  const p = auditPath();
  if (!existsSync(p)) return [];
  const out = [];
  for (const line of readFileSync(p, 'utf8').split('\n')) {
    if (!line.trim()) continue;
    try { out.push(JSON.parse(line)); } catch { /* bỏ dòng hỏng */ }
  }
  return out;
}
