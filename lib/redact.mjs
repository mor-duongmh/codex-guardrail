...
const PATTERNS = [
  ...
  {
    // Tiền tố là TUỲ CHỌN nhưng nếu có thì phải kết bằng `_`. Bản trước dùng
    // `(.*?_(...))` tức BẮT BUỘC có gạch dưới, nên `DB_PASSWORD=x` được che còn
    // `PASSWORD=x` LỌT — đúng dạng tên biến phổ biến nhất lại vào audit log ở
    // dạng thô.
    // KHÔNG sửa thành `(.*?(...))=`: thế thì `MONKEY=banana` và `BYPASS=1` bị
    // che oan vì chỉ vô tình kết thúc bằng `KEY`/`PASS`. Đòi tiền tố kết bằng
    // `_` loại đúng nhóm đó mà vẫn nhận `AWS_ACCESS_KEY`.
    // Danh sách từ khoá phải TRÙNG với SENSITIVE_ENV_VARS trong rules/secrets.mjs.
    // Đo được lúc lệch: `printenv PASSWD` và `printenv APIKEY` bị rule CHẶN,
    // nhưng `PASSWD=x` / `APIKEY=x` KHÔNG được che — tức guardrail coi biến đó
    // đủ nhạy cảm để chặn, rồi tự ghi giá trị của nó vào audit log ở dạng thô.
    // Thứ tự alternation: dạng dài đứng trước dạng ngắn hơn nó chứa
    // (`PASSWORD`/`PASSWD` trước `PASS`, `APIKEY` trước `KEY`).
    kind: 'env-sensitive',
    re: new RegExp(
      '\\b((?:[A-Za-z0-9]+_)*'
      + '(SECRET|TOKEN|PASSWORD|PASSWD|APIKEY|CREDENTIALS|KEY|PASS))='
      + valueSrc(),
      'gi',
    ),
    mask: '$1=***',
