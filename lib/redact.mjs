const PATTERNS = [
  { kind: 'aws-access-key-id', re: /AKIA[0-9A-Z]{16}/g, mask: 'AKIA***' },
  { kind: 'openai-key', re: /\bsk-[A-Za-z0-9_-]{20,}/g, mask: 'sk-***' },
  { kind: 'github-token', re: /\b(?:ghp|gho|ghu|ghs)_[A-Za-z0-9]{20,}/g, mask: 'gh_***' },
  { kind: 'github-pat', re: /\bgithub_pat_[A-Za-z0-9_]{20,}/g, mask: 'github_pat_***' },
  { kind: 'slack-token', re: /\bxox[baprs]-[A-Za-z0-9-]{10,}/g, mask: 'xox_***' },
  {
    kind: 'jwt',
    re: /\beyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]+/g,
    mask: 'jwt.***',
  },
  {
    kind: 'private-key',
    re: /-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----/g,
    mask: '***private-key***',
  },
  {
    kind: 'url-userinfo',
    re: /([a-z][a-z0-9+.-]*):\/\/([^:\/]+):([^@]+)@/gi,
    mask: '$1://$2:***@',
  },
  // Cả 3 pattern -password* dùng chung nguyên tắc: nếu giá trị được bọc
  // trong quote, bắt tới quote đóng tương ứng (có thể chứa khoảng trắng bên
  // trong); nếu không, dừng ở khoảng trắng (hoặc = với -p) như cũ. Thiếu
  // nhánh quote là lỗ đã rò secret ba round liền — sửa đồng bộ cả 3 nơi.
  {
    kind: 'password-flag-long',
    re: /--password=(?:'[^']*'|"[^"]*"|[^\s]+)/g,
    mask: '--password=***',
  },
  {
    kind: 'password-flag-space',
    re: /--password\s+(?:'[^']*'|"[^"]*"|[^\s]+)/g,
    mask: '--password ***',
  },
  {
    kind: 'password-flag-p',
    re: /(?<![a-z-])-p(?:'[^']*'|"[^"]*"|[^\s=]+)/g,
    mask: '-p***',
  },
  {
    kind: 'env-sensitive',
    re: /\b(.*?_(SECRET|TOKEN|PASSWORD|KEY|CREDENTIALS|PASS))=([^\s]+)/gi,
    mask: '$1=***',
  },
  {
    kind: 'auth-header',
    re: /Authorization:\s*(Bearer|Basic)\s+([^\s]+)/gi,
    mask: 'Authorization: $1 ***',
  },
];

export function redact(text) {
  let out = String(text ?? '');
  for (const { re, mask } of PATTERNS) out = out.replace(re, mask);
  return out;
}

export function findSecretKinds(text) {
  const s = String(text ?? '');
  return PATTERNS.filter(({ re }) => new RegExp(re.source, re.flags).test(s)).map(p => p.kind);
}
