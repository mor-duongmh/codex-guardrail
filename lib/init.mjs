...
export function initProject({ cwd, runGit = defaultRunGit, force = false }) {
  ...
  lines.push('');
  lines.push('Còn MỘT bước nữa — đây là tầng duy nhất mà dev KHÔNG tháo được:');
  lines.push('');
  lines.push('  1. Thêm vào CODEOWNERS (tầng 3 — nới policy phải có lead duyệt):');
  lines.push(`       ${POLICY_FILE} @<lead-github-username>`);
  lines.push('');
  lines.push('Tầng hook local ai cũng tháo được — bước trên tồn tại chính vì thế.');
  lines.push('');
  // Bản trước in ra "Thêm bước gọi `guardrail ci` vào workflow PR" như một việc
  // cần làm, nhưng `bin/guardrail.mjs` KHÔNG có nhánh `ci` — nó in USAGE và
  // thoát 1. Dạy mọi dev thêm một bước CI luôn đỏ là dạy họ bỏ qua hướng dẫn
  // của init. Tầng 2 chưa có thì nói là chưa có, và nói luôn hệ quả.
  lines.push('Tầng 2 (bước CI gác PR) CHƯA có trong bản này: subcommand');
  lines.push('`guardrail ci` chưa được cài đặt, nên đừng thêm bước CI nào gọi');
  lines.push('nó — nó sẽ luôn đỏ. Tới khi có, CODEOWNERS ở trên là tầng duy nhất.');
