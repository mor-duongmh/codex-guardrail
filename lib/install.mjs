...
export function checkCodexCli(env = process.env) {
  // `delimiter` của node:path, KHÔNG hardcode ':'. Bản đầu dùng `split(':')` và
  // CI ma trận bắt được ngay lần chạy ĐẦU TIÊN: trên Windows PATH phân tách bằng
  // ';', mà mỗi đường dẫn lại CHỨA ':' (`C:\...`), nên tách bằng ':' cho ra rác —
  // hàm luôn trả 'missing' và doctor báo sai trên MỌI máy Windows.
  //
  // Tên file cũng theo nền tảng: Windows cần `codex.exe`/`.cmd`/`.bat`. Vẫn thử
  // cả `codex` không đuôi để không bỏ sót bản cài qua shim.
  ...
  for (const dir of String(env.PATH ?? '').split(delimiter).filter(Boolean)) {
    const p = names
      .map(n => join(dir, n))
      .find((c) => { try { lstatSync(c); return true; } catch { return false; } });
    if (!p) continue;
