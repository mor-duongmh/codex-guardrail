...
export function buildContext(rawStdin, env = {}) {
  ...
  return {
    ...
    escapes,
    // Chế độ quyền của phiên. Enum đầy đủ lấy từ JSON Schema NHÚNG TRONG binary
    // codex 0.150.1: "default" | "acceptEdits" | "plan" | "dontAsk" |
    // "bypassPermissions".
    //
    // Cần cho quyết định `ask`: một `ask` chỉ có nghĩa ở chế độ thật sự có người
    // trả lời. Ở `dontAsk`/`bypassPermissions` thì rule phải hạ `ask` về `deny`
    // thay vì tin rằng có prompt (spec deploy §3.9).
    //
    // THIẾU thì để null, KHÔNG đoán 'default': đoán 'default' là đoán rằng prompt
    // sẽ hiện, tức lệch về phía CHO QUA ở đúng chỗ không được lệch.
